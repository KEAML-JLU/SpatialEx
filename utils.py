...
def structural_similarity_on_graph_data(x, y, adj, K1=0.01, K2=0.03, alpha=1, beta=1, gamma=1, sigma=1.5, use_sample_covariance=True):
    ...
    if metric == 'cosine_similarity':
        ...
        if x.shape[0] < 200000:
            ...
            for i in tqdm(range(batch_num)):
                ...
                metric = structural_similarity_on_graph_data(x[tgt_cells], x_prime[tgt_cells], graph[tgt_cells][:, tgt_cells])
...
def Plot_imputed_gene(adata, gene_name, downsample=50000, save_path=None, dpi=300):
    ...
    sc1 = axs[0].scatter(adata.obs['x_centroid'], adata.obs['y_centroid'], c=adata[:, gene_name].X, cmap='viridis', vmin=0, vmax=vmax, s=30) 
    axs[0].axis('equal')
    axs[0].axis('off')
    axs[0].set_title(gene_name + ' Ground Truth', fontsize=40)
    
    adata.X = adata.obsm['x_prime']
    sc2 = axs[1].scatter(adata.obs['x_centroid'], adata.obs['y_centroid'], c=adata[:, gene_name].X, cmap='viridis', vmin=0, vmax=vmax, s=30)
