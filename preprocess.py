...
def Cell_segmentation(img, percent_threshold=0.2, pixel_threshold=190, device='cpu',
    ...
    **diameter**:   int,    The median of the diameter of detected cells
    ...
    patches = image_tensor_padded.unfold(2, patch_size, patch_size).unfold(3, patch_size, patch_size) # 使用unfold将图像切分成patches
...
def Extract_HE_patches_representaion(args, he_patches, store_key=None, adata=None, skip_embedding=False):
    ...
    if not skip_embedding:
        ...
        for i in tqdm(range(batch_num)):
            img_tensor = preprocess(he_patches[i*args.img_batch_size:min((i+1)*args.img_batch_size,he_patches.size(0))].to(args.device))
            with torch.no_grad():
        ...
        for i in tqdm(range(batch_num)):
            img_tensor = preprocess(he_patches[i*args.img_batch_size:min((i+1)*args.img_batch_size,he_patches.size(0))])
...
def Build_graph_for_high_dim_feat(x, weighted=False, num_neighbors=50, device='cpu', batch_size=4096, apply_normalize=False, type='coo'):
...
def Build_hypergraph(x, metric='euclidean', graph_type='knn', radius=50, num_neighbors=50, 
    ...
    H = Build_graph(x, metric=metric, graph_type=graph_type, radius=radius, num_neighbors=num_neighbors, self_loop=self_loop, type=type)
    H = H.T
    if normalize:
...
def Build_hypergraph_spatial_and_HE(args, adata, normalize=False, graph_kind='spatial', type='coo'):
    if graph_kind.lower() == 'spatial':
        H1 = Build_graph(adata.obsm['spatial'], graph_type='knn', num_neighbors=args.num_neighbors, type=type)
        H = H1.T
        # H = H1
        ...
        H1 = Build_graph(adata.obsm['spatial'], graph_type='knn', num_neighbors=args.num_neighbors)
        H2 = Build_graph_for_high_dim_feat(adata.obsm['he'], num_neighbors=args.num_neighbors, 
                                       device=args.device, batch_size=args.batch_size)
...
def Build_dataloader(adata, graph, batch_size=1, ori=False, graph_norm='hpnn', feat_norm=False, 
    ...
    dataset = Xenium_HBRC_overlap(adata, graph, ori=ori, prune=prune, graph_norm=graph_norm, feat_norm=feat_norm, drop_last=drop_last)
...
class Xenium_HBRC_overlap(torch.utils.data.Dataset):
    def __init__(self, adata, graph, ori=False, graph_norm='hpnn', feat_norm=False, prune=[3000, 3000], drop_last=False):
        ...
        for name in idx_cat:
            ...
            selection = (row>x+0.5*prune[0]) & (row<(x+1.5*prune[0])) & (col>y+0.5*prune[1]) & (col<(y+1.5*prune[1]))
